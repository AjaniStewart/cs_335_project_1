...
AvlTree<Comparable>::printTreeToStream( std::ostream& os, AvlNode<Comparable> *t ) const {
  ...
  return printTreeToStream(os,t->left) << t->element << printTreeToStream(os,t->right);
}
...
AvlTree<Comparable>::collectIntoListIf( std::function<bool (Comparable)> p, AvlNode<Comparable> *t ) const {
  if (NULL == t) {
    ...
    if (p(t->element)) {
      ...
      p.merge(collectIntoListIf(p, t->right));
    } else {
      return (collectIntoListIf(p, t->left).merge(collectIntoListIf(p, t->right)));
    }
  }
}